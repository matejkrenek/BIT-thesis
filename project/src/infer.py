...
def main() -> None:
    ...
    print(f"[inference] input points: {result.input_info['points']}")
    print(f"[inference] output points: {result.output_info['points']}")
    for stage in result.stage_timings:
        print(f"[timing] {stage.name}: {stage.seconds:.4f}s")
