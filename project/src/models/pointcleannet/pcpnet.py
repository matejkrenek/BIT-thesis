...
class STN(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.quaternion:
            ...
            iden = torch.tensor([1.0, 0.0, 0.0, 0.0], dtype=x.dtype, device=x.device)
            x = x + iden

            # convert quaternion to rotation matrix
            trans = torch.empty(batchsize, 3, 3, dtype=x.dtype, device=x.device)
