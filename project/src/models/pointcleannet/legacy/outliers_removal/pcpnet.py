...
class STN(nn.Module):
    def __init__(
        self, num_scales=1, num_points=500, dim=3, sym_op="max", quaternion=False
    ):
    ...
    def forward(self, x):
        ...
        if self.num_scales == 1:
            ...
            if x.is_cuda:
                x_scales = Variable(
                    torch.cuda.FloatTensor(x.size(0), 1024 * self.num_scales, 1)
                )
            else:
                x_scales = Variable(
                    torch.FloatTensor(x.size(0), 1024 * self.num_scales, 1)
                )
        ...
        if not self.quaternion:
            iden = (
                Variable(torch.from_numpy(np.identity(self.dim, "float32")).clone())
...
class PointNetfeat(nn.Module):
    def __init__(
        ...
        if self.use_point_stn:
            ...
            self.stn1 = STN(
                ...
                num_points=num_points * self.point_tuple,
                dim=3,
                sym_op=self.sym_op,
                quaternion=True,
            )
    ...
    def forward(self, x):
        ...
        if self.get_pointfvals:
            ...
            pointfvals = (
                None  # so the intermediate result can be forgotten if it is not needed
            )
        ...
        if self.num_scales == 1:
            ...
            if x.is_cuda:
                x_scales = Variable(
                    torch.cuda.FloatTensor(x.size(0), 1024 * self.num_scales**2, 1)
                )
            else:
                x_scales = Variable(
                    torch.FloatTensor(x.size(0), 1024 * self.num_scales**2, 1)
                )
            if self.sym_op == "max":
                for s in range(self.num_scales):
                    x_scales[
                        :,
                        s * self.num_scales * 1024 : (s + 1) * self.num_scales * 1024,
                        :,
                    ] = self.mp1(
                        x[:, :, s * self.num_points : (s + 1) * self.num_points]
                    )
