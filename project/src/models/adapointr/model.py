...
from .transformer import (
    ...
    DeformableLocalCrossAttention,
    DynamicGraphAttention,
    LayerScale,
    Mlp,
    improvedDeformableLocalGraphAttention,
    index_points,
    knn_point,
)
from .utils import ChamferDistanceL1, fps, fps_indices, gather_operation, jitter_points
...
class AdaPoinTr(nn.Module):
    ...
    def get_loss(self, ret, gt, epoch=1):
        if isinstance(ret, (tuple, list)) and len(ret) == 4:
            ...
            # denoise loss (training-time output)
        ...
        if isinstance(ret, (tuple, list)) and len(ret) == 2:
            # eval-time output has no denoise branch
