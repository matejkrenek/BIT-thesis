...
import einops
from .utils import index_points, knn_point, three_interpolate, three_nn
...
class DeformableLocalAttention(nn.Module):
    ...
    def forward(self, x, pos, idx=None):
        ...
        assert (
            len(pos.shape) == 3 and pos.size(-1) == 3
        ), f"[ERROR] Got an unexpected shape for pos, expect it to be B N 3, but got {pos.shape}"
        ...
        off_local_v = einops.rearrange(
            local_v, "b n k (g c) -> (b g) n k c", g=self.n_group, c=self.group_dims
        )  # Bg N k c
        group_q = einops.rearrange(
            q, "b n (g c) -> (b g) n c", g=self.n_group, c=self.group_dims
        )  # Bg N c
        # calculate offset
        shift_feat = torch.cat(
            [off_local_v, group_q.unsqueeze(-2).expand(-1, -1, self.k, -1)], dim=-1
        )  # Bg N k 2c
        ...
        local_v_pos = local_v_pos.unsqueeze(1).expand(
            -1, self.n_group, -1, -1, -1
        )  # B g N k 3
        local_v_pos = einops.rearrange(
            local_v_pos, "b g n k c -> (b g) n k c"
        )  # Bg N k 3
        ...
        v = einops.rearrange(
            x, "b n (g c) -> (b g) n c", g=self.n_group, c=self.group_dims
        )  # Bg N c
        # three_nn and three_interpolate
        dist, _idx = three_nn(
            shift_pos.contiguous(), pos.contiguous()
        )  #  Bg k*N 3, Bg k*N 3
        dist_reciprocal = 1.0 / (dist + 1e-8)
        norm = torch.sum(dist_reciprocal, dim=2, keepdim=True)
        weight = dist_reciprocal / norm
        interpolated_feats = (
            three_interpolate(v.transpose(-1, -2).contiguous(), _idx, weight)
            .transpose(-1, -2)
            .contiguous()
        )
        interpolated_feats = einops.rearrange(
            interpolated_feats,
            "(b g) (n k) c  -> b n k (g c)",
            b=B,
            g=self.n_group,
        ...
        q = einops.rearrange(
            local_q, "b n k (h c) -> (b h n) k c", h=self.num_heads, c=self.head_dim
        )  # BHN k c
        k = self.proj_k(interpolated_feats)
        k = einops.rearrange(
            k, "b n k (h c) -> (b h n) k c", h=self.num_heads, c=self.head_dim
        )  # BHN k c
        v = self.proj_v(interpolated_feats)
        v = einops.rearrange(
            v, "b n k (h c) -> (b h n) k c", h=self.num_heads, c=self.head_dim
        )  # BHN k c
        ...
        out = einops.rearrange(
            out, "(b h n) k c -> b n k (h c)", b=B, n=N, h=self.num_heads
        )  # B N k C
...
class DeformableLocalCrossAttention(nn.Module):
    ...
    def forward(self, q, q_pos, v=None, v_pos=None, idx=None, denoise_length=None):
        ...
        if denoise_length is None:
            ...
            # given N token and pos
            assert (
                len(v_pos.shape) == 3 and v_pos.size(-1) == 3
            ), f"[ERROR] Got an unexpected shape for v_pos, expect it to be B N 3, but got {v_pos.shape}"
            assert (
                len(q_pos.shape) == 3 and q_pos.size(-1) == 3
            ), f"[ERROR] Got an unexpected shape for q_pos, expect it to be B N 3, but got {q_pos.shape}"
            ...
            # interpolate
            shift_pos = einops.rearrange(
                shift_pos, "bg n k c -> bg (n k) c"
            )  # Bg k*N 3
            v_pos = v_pos.unsqueeze(1).expand(-1, self.n_group, -1, -1)  # B g Nk 3
            v_pos = einops.rearrange(v_pos, "b g n c -> (b g) n c")  # Bg Nk 3
            v = einops.rearrange(
                v, "b n (g c) -> (b g) n c", g=self.n_group, c=self.group_dims
            )  # Bg Nk c
            # three_nn and three_interpolate
            dist, idx = three_nn(
                shift_pos.contiguous(), v_pos.contiguous()
            )  #  Bg k*N 3, Bg k*N 3
            dist_reciprocal = 1.0 / (dist + 1e-8)
            norm = torch.sum(dist_reciprocal, dim=2, keepdim=True)
            weight = dist_reciprocal / norm
            interpolated_feats = (
                three_interpolate(v.transpose(-1, -2).contiguous(), idx, weight)
                .transpose(-1, -2)
                .contiguous()
            )
            interpolated_feats = einops.rearrange(
            ...
            # interpolated_feats : B N k C
            q = einops.rearrange(
                q, "b n (h c) -> (b h n) c", h=self.num_heads, c=self.head_dim
            ).unsqueeze(
                -2
            )  # BHN 1 c
            k = self.proj_k(interpolated_feats)
            ...
            assert (
                idx is None
            ), f"we need online index calculation when denoise_length is set, denoise_length {denoise_length}"
            # when v_pos and v are given, that to say, it's a cross attn.
            # we only consider self-attn
            assert (
                v is None
            ), f"mask for denoise_length is only consider in self-attention, but v is given"
            assert (
                v_pos is None
            ), f"mask for denoise_length is only consider in self-attention, but v_pos is given"
            ...
            idx = knn_point(
                self.k, v_pos[:, :-denoise_length], q_pos[:, :-denoise_length]
            )  # B N_r k
            assert idx.size(-1) == self.k
...
class improvedDeformableLocalGraphAttention(nn.Module):
    ...
    def forward(self, q, q_pos, v=None, v_pos=None, idx=None, denoise_length=None):
        ...
        if denoise_length is None:
            ...
            # calculate offset
            shift_feat = torch.cat(
                [off_local_v, q.unsqueeze(-2).expand(-1, -1, self.k, -1)], dim=-1
            )  # B N k 2c
            ...
            interpolated_feats = einops.rearrange(
                interpolated_feats, "b (n k) c  -> b n k c", n=N, k=self.k
            )  # B N k c
...
class RegionWiseBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.deformable_attn = DeformableLocalAttention(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            attn_drop=attn_drop,
...
class DeformableAttnBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.deformable_attn = DeformableLocalCrossAttention(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            attn_drop=attn_drop,
...
class DeformableAttnDecoderBlock(nn.Module):
    ...
    def forward(self, q, v, q_pos, v_pos):
        ...
        q = q + self.drop_path2(
            self.ls2(
                self.attn(q=self.norm_q(q), v=self.norm_v(v), q_pos=q_pos, v_pos=v_pos)
            )
        )
        q = q + self.drop_path3(self.ls3(self.mlp(self.norm2(q))))
        return q
